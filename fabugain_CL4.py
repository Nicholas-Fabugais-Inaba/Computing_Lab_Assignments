...
"""
someList = ['BX031112', ['0.7', '2019-01-02'], ['0.97', '2019-02-13']]
...
def passedTestsBX(testInfoBX):
    if (getTestScores(testInfoBX)[0])>=0.7:
        if (getTestMonths(testInfoBX)[1])==1 or (getTestMonths(testInfoBX)[1])==2 and (getTestScores(testInfoBX)[1])>=0.98:
            return True
        elif (getTestMonths(testInfoBX)[1])==3 and (getTestScores(testInfoBX)[1])>=0.9:
            return True
        elif (getTestMonths(testInfoBX)[1])==4 and (getTestScores(testInfoBX)[1])>=0.87:
            return True
        elif (getTestMonths(testInfoBX)[1])>4 and (getTestMonths(testInfoBX)[1])<13 and (getTestScores(testInfoBX)[1])>=0.85:
            return True
        else:
            return False
    else:
...
def determinePassedProducts(products):
    ...
    for i in products:
        if getProductVersion(i) == "GX":
            if passedTestsGX(i) == True:
                final.append(i[0])
            else:
                continue
        elif getProductVersion(i) == "BX":
            if passedTestsBX(i) == True:
                final.append(i[0])
            else:
                continue
    return final
...
# ```
# It is not necessary to use a for loop in the getTestScores and getTestMonths function because the index positions of 1 and 2 can be stated instead of having to create a for loop that runs for the first and second index positions. It is beneficial to use a for loop as it reduces the amount of lines that needs to be written. If the number of quality assurance tests is unknown, a for loop should be used as it would be very inefficient to write for multiple tests and if the number is unknown, there wouldn't be a way to tell how much code would need to be written for the amount of tests.
# It is possible to use a while loop, however, there would not be much of a benefit to use one. Not entirely sure of a scenario where a while loop would be more beneficial.
# 
# ```
