...
def extractGrade(x):
    list = x.split("-")
    return float(list[-1].strip("%"))
def classAverage(final_marks):
    sum = 0.0
    for i in final_marks:
        sum += extractGrade(i)
    return sum/len(final_marks)
def classStdDev(final_marks):
    sum = 0.0
    for i in final_marks:
        sum += (extractGrade(i) - classAverage(final_marks))**2
    return math.sqrt(sum/len(final_marks))
...
# ```
# There is no input that can cause your program to fail.
# Instead of splitting the function by "-", it should be split by ":" and after, there needs to be no strip function as there is no "%" to get rid of so all that would need to be returned is the last position of the list ie. return float(list[-1])
# No you cannot use a single for loop because in order to calculate the standard deviation, you need the average, which needs to take in all of the values in the list, in a separate for loop, before being applied to the std deviation calculation. 
# 
# ```
